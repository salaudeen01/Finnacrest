  ...
  return (
    <div elevation={6} className="px-6 py-5 h-full" style={style.card} ref={element => { 
      ...
      <Grid container >
        ...
        <Typography variant="h6">{props.amount == numberFormat(0)? "": props.amount}</Typography>
        </Grid>
        <Grid item lg={6} md={6} sm={12}>
        <Typography variant="h6">{props.investment_amount == numberFormat(0)? "": props.investment_amount}</Typography>
        </Grid>
      </Grid>
    </div>
  );
