...
import { Grid, Card, Button, Divider, Typography, IconButton, Toolbar, AppBar, Dialog, MenuItem, TextField, Checkbox} from '@material-ui/core';
...
fetchSingleMarket(id){
  ...
  fetch(getConfig('getMarketNews')+ id, requestOptions)
        .then(async response => {
    ...
    fetch(getConfig('getSingleInvestment')+ id, requestOptions)
        .then(async response => {
        const data = await response.json();
        if (!response.ok) {
            const error = (data && data.message) || response.statusText;
...
handleChange(event) {
  ...
  if(name == "slots"){
    this.setState({invest_data: { ...invest_data, [name]: value, total:value*current_value }})
...
tabbed = (id) => {
  ...
  render(){
    ...
    const {isLoading, tab, mTab, categories, category, cards, add_card, showSaveCard, loading, current_index, investment, singleInvestment, singleNews, showView, showInvest, invest_data} = this.state
    return (
                ...
                <Card className="px-6 pt-2 pb-4">
                    <Grid container spacing={2}>
                      <Grid item lg={12} md={12} sm={12} xs={12}>
                        ...
                        {(invest_data.payment_method == "Wallet" || (invest_data.card_id !="0" && invest_data.card_id !="")) && 
                      ...
                      <Grid item lg={12} md={12} sm={12} xs={12}>
                        ...
                        <PayCard cards={cards} value={invest_data.card_id} open={(e)=>this.setState({ invest_data:{...invest_data, card_id:""}})}  handleChange={this.handleChange}/>
                      </Grid>}
